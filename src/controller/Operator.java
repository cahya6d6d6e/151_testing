...
public class Operator {
	...
	public static ResultSet getListDetilTransaksiMasuk(Connection con, int idTransaksi) {
		return select(con, "SELECT * FROM trans_masuk_detail WHERE id_masuk=" + idTransaksi);
	}
	...
	public static ResultSet getListDetilTransaksiKeluar(Connection con, int idTransaksi) {
		return select(con, "SELECT * FROM trans_keluar_detail WHERE id_keluar=" + idTransaksi);
	}
	...
	public static boolean tambahProduk(Produk brg, Connection con) {
		return update(con, "INSERT INTO produk " + "VALUES(" + (getLastIDProduk(con) + 1) + ",'" + brg.getNama() + "',"
				+ brg.getJumlah() + "," + brg.getHargaJual() + "," + brg.getHargaBeli() + ")");
	}

	public static boolean tambahKaryawan(Karyawan karyawan, Connection con) {
		return update(con,
				"INSERT INTO karyawan " + "VALUES(" + (getLastIDKaryawan(con) + 1) + ",'" + karyawan.getNama() + "','"
						+ karyawan.getAlamat() + "','" + karyawan.getTelp() + "','" + karyawan.getPosisi() + "','"
						+ karyawan.getPassword().toString() + "')");
	}

	public static boolean tambahSupplier(Supplier supplier, Connection con) {
		return update(con, "INSERT INTO supplier " + "VALUES(" + (getLastIDSupplier(con) + 1) + ",'"
				+ supplier.getNama() + "','" + supplier.getAlamat() + "','" + supplier.getTelp() + "')");
	}
	...
	public static boolean tambahTransaksiMasuk(TransaksiMasuk trns, Connection con) {
		boolean returnValue = true;
		int id = getLastIDTransMasuk(con) + 1;
		returnValue = update(con, "INSERT INTO trans_masuk VALUES(" + id + ", " + trns.getUser().getId() + ", "
				+ trns.getSupplier().getId() + ", '" + trns.getTglAsString() + "', '" + trns.getTotalHrg() + "')");
		for (int i = 0; i < trns.getDetilTransaksi().size(); i++) {
			returnValue = returnValue & update(con,
					"INSERT INTO trans_masuk_detail " + "VALUES(" + id + ", "
							+ + +trns.getDetilTransaksi().get(i).getBarang().getId() + ", "
							+ trns.getDetilTransaksi().get(i).getJumlah() + ")");
			returnValue = returnValue
					& update(con, "UPDATE produk SET stok = stok+" + trns.getDetilTransaksi().get(i).getJumlah()
							+ " WHERE id_produk=" + trns.getDetilTransaksi().get(i).getBarang().getId());
		}
		return returnValue;
	}

	public static boolean tambahTransaksiKeluar(TransaksiKeluar trns, Connection con) {
		boolean returnValue = true;
		int id = getLastIDTransKeluar(con) + 1;
		returnValue = update(con, "INSERT INTO trans_keluar VALUES(" + id + ", " + trns.getUser().getId() + ", '"
				+ trns.getTglAsString() + "', '" + trns.getUser().getId() + "')");
		for (int i = 0; i < trns.getDetilTransaksi().size(); i++) {
			returnValue = returnValue & update(con,
					"INSERT INTO trans_keluar_detail " + "VALUES(" + id + ", "
							+ + +trns.getDetilTransaksi().get(i).getBarang().getId() + ", "
							+ trns.getDetilTransaksi().get(i).getJumlah() + ")");
			returnValue = returnValue
					& update(con, "UPDATE produk SET stok = stok-" + trns.getDetilTransaksi().get(i).getJumlah()
	...
	public static boolean hapusTransaksiMasuk(TransaksiKeluar trns, Connection con) {
		boolean returnData = true;
		if (trns == null) {
			return false;
		} else {
			returnData = returnData & update(con, "DELETE FROM trans_masuk WHERE id_masuk=" + trns.getId());
			returnData = returnData & update(con, "DELETE FROM trans_masuk WHERE id_masuk=" + trns.getId());
		}
		return returnData;
	}

	public static boolean hapusTransaksiKeluar(TransaksiKeluar trns, Connection con) {
		boolean returnData = true;
		if (trns == null) {
			return false;
		} else {
			returnData = returnData & update(con, "DELETE FROM trans_keluar WHERE id_keluar=" + trns.getId());
			returnData = returnData & update(con, "DELETE FROM trans_keluar WHERE id_keluar=" + trns.getId());
		}
		return returnData;
	}
	...
	public static Karyawan login(Connection con, String id_karyawan, String password) {
		ResultSet rs = select(con, "SELECT * FROM karyawan " + "WHERE id_karyawan='" + id_karyawan + "'");
		try {
			if (rs.next()) {
				if (rs.getString("password").equals(password)) {
					return new Karyawan(rs.getInt("id_karyawan"), rs.getString("nama_karyawan"),
							rs.getString("alamat_karyawan"), rs.getString("telp_karyawan"), rs.getString("posisi"),
