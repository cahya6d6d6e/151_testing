...
public class WindowFormKaryawan extends JFrame {
	...
	public WindowFormKaryawan(Core core) {
		...
		try {
			while (rs.next()) {
				karyawan.add(new Karyawan(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
		...
		tbl = new JTable(Operator.resultSetToTableModel(Operator.getListKaryawan(core.getConnection())));
		...
		buttonTambah.addActionListener(
				new CustActionListener(core, this, tbl, buttonTambah, CustActionListener.INSERT_KARYAWAN));
		buttonTambah.setBounds(205, 135, 80, 20);

		JButton buttonDelete = new JButton("Delete");
		buttonDelete.addActionListener(
				new CustActionListener(core, this, tbl, buttonTambah, CustActionListener.DELETE_KARYAWAN));
	...
	public void submitToDB() {
		...
		((DefaultTableModel) tbl.getModel()).addRow(new Object[] { Operator.getLastIDKaryawan(core.getConnection()),
				tfNama.getText(), tfAlamat.getText(), tfTelp.getText(), tfPosisi.getText(), tfPassword.getPassword() });
		tfNama.setText("");
		tfAlamat.setText("");
		tfTelp.setText("");
		tfPosisi.setText("");
	...
	public Karyawan getKaryawan() {
		return new Karyawan(tfNama.getText(), tfAlamat.getText(), tfPosisi.getText(), tfTelp.getText(),
