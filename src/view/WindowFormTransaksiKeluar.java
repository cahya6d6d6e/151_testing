    ...
    public WindowFormTransaksiKeluar(Core core) {
        ...
        try {
            while (rs_barang.next()) {
                barang.add(new Produk(
                        rs_barang.getInt(1),
                        rs_barang.getString(2),
                        rs_barang.getInt(3),
                        rs_barang.getInt(4),
                        rs_barang.getInt(5)));
        ...
        try {
            while (rs_supplier.next()) {
                supplier.add(new Supplier(
                        rs_supplier.getInt(1),
                        rs_supplier.getString(2),
                        rs_supplier.getString(3),
                        rs_supplier.getString(4)));
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this, e.getMessage());
        }
        cbSupplier = new JComboBox(nmBarang);

        tfTgl = new JTextField(core.getDateAsString());
        tfKasir = new JTextField(user.getId());
...
//
//        JMenu menuUser = new JMenu(core.getLoggedInUser().getNama());
...
//        // JMenuItem miBarangCetak = new JMenuItem("Cetak Produk");
//
//        menu.add(menuUser);
...
//        menuBarang.add(miBarangData);
    ...
    public void fillFormByIndex(int index) {
        tfJumlah.setText("1");
        tfHarga.setText(barang.get(index).getHargaBeli()
